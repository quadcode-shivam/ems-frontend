...
function DemoNavbar(props) {
  ...
  const [color, setColor] = React.useState("#11171D");
  const sidebarToggle = React.useRef();

  const toggle = () => {
    setIsOpen(!isOpen);
    setColor(isOpen ? "#11171D" : "#4a5a5f"); 
  };
  ...
  React.useEffect(() => {
    const updateColor = () => {
      setColor(window.innerWidth < 993 && isOpen ? "#4a5a5f" : "#11171D"); // Adjust color on resize
    };
  ...
  return (
    <Navbar
      style={{ backgroundColor: location.pathname.includes("full-screen-maps") ? "#11171D" : color }}
      expand="lg"
